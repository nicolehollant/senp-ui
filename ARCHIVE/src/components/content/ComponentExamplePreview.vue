<template>
  <div>
    <div class="mt-8 mb-4 flex w-max bg-gray-950 border border-gray-800 gap-1 rounded-lg p-1">
      <SenpButton
        size="sm"
      ...
      <SenpButton
        size="sm"
        :intent="state.componentBackground === 'bright' ? 'primary' : 'secondary'"
      ...
      <SenpButton
        size="sm"
        :intent="state.componentBackground === 'white' ? 'primary' : 'secondary'"
      ...
      <SenpButton
        size="sm"
        :intent="state.componentBackground === 'black' ? 'primary' : 'secondary'"
    ...
    <div
      ...
      :class="{
        ...
        'border border-white bg-gray-300': state.componentBackground === 'bright',
        'border border-gray-300 bg-white': state.componentBackground === 'white',
        'border border-gray-800 bg-black': state.componentBackground === 'black',
      }"
    >
      <Component :is="Comp"></Component>
    </div>
    <div class="mt-8 mb-4 flex w-max bg-gray-950 border border-gray-800 gap-1 rounded-lg p-1">
      <SenpButton size="sm" :intent="state.mode === 'props' ? 'primary' : 'secondary'" @click="state.mode = 'props'">
        props
      </SenpButton>
      <SenpButton size="sm" :intent="state.mode === 'slots' ? 'primary' : 'secondary'" @click="state.mode = 'slots'">
        slots
      </SenpButton>
      <SenpButton size="sm" :intent="state.mode === 'code' ? 'primary' : 'secondary'" @click="state.mode = 'code'">
        code
      </SenpButton>
    </div>
    ...
    <div v-if="state.mode === 'props' && filteredProps" class="not-prose">
      ...
      >
        <tr v-for="(row, i) in filteredProps" :key="'row-' + i">
          ...
          <td>
            <div class="max-w-xs text-xs" style="display: flex; flex-direction: column; gap: 0.75rem">
              <pre class="whitespace-pre-wrap !leading-snug"><code v-if="row.type" style="color: #93c5fd;">{{
              ...
              <p class="m-0" v-if="row.defaultValue">
    ...
    <div v-else-if="state.mode === 'props'" style="color: #d4d4d4; font-size: 0.875rem">
    ...
    <div v-if="state.mode === 'slots' && filteredSlots" class="not-prose">
      ...
      >
        <tr v-for="(row, i) in filteredSlots" :key="'row-' + i">
          ...
          <td>
            <pre class="text-[10px] text-blue-300"><code>{{ row.bindings }}</code></pre>
          </td>
          <td style="width: 20rem">
            <div style="width: 20rem; position: relative">
              <StoryControl
    ...
    <div v-else-if="state.mode === 'slots'" style="color: #d4d4d4; font-size: 0.875rem">
...
const codePreview = computed(() => {
  ...
  const propStr = Object.entries(_props)
    .map(([key, value]) => {
      let defaultValue = filteredProps.value?.find((a) => a.name === key)?.defaultValue
      if (defaultValue?.startsWith("'") && defaultValue?.endsWith("'")) {
        defaultValue = defaultValue.slice(1, -1)
      }
      console.log(`default value for ${key} is `, defaultValue, 'found', value === undefined ? 'undefined' : value)
  ...
  const slots = _slots
    ? Object.entries(_slots)
        .map(([slot, content]) => (content == null ? null : `  <template #${slot}>${content}</template>`))
        .filter(Boolean)
        .join('\n')
    : ''
  const slotContent =
  ...
  return `<${[props.docgenInfo.displayName, propStr].filter(Boolean).join(' ')}${slotContent}`
})
