---
description: Simple drag and drop lists using vuedraggable
---

# DraggableList

Simple drag and drop lists using [vuedraggable](https://github.com/SortableJS/vue.draggable.next)

---

## Usage

Use `v-model` to make the `DraggableList` reactive.

::ComponentExample
---
is: ExampleDraggableList
---
#code
```vue
<template>
  <div class="grid grid-cols-2 gap-4">
    <div class="grid gap-4 grid-rows-[auto,minmax(0,1fr)] h-full" v-for="(_, key) in columns">
      <h3 class="text-xl">{{ key }}</h3>
      <SenpDraggableList
        v-model="columns[key]"
        class="flex flex-col gap-2 overflow-auto h-full"
        :classes="{ elementWrapper: { base: 'w-full' } }"
      >
        <template #item="{ element }">
          <SenpCard
            class="cursor-move"
            :classes="{ wrapper: { extend: key === 'Done' ? 'opacity-50 !bg-gray-950' : '' } }"
            :key="element.id"
            type="header"
            :title="element.title"
          >
            <p class="text-sm line-clamp-3">{{ element.description }}</p>
          </SenpCard>
        </template>
      </SenpDraggableList>
    </div>
