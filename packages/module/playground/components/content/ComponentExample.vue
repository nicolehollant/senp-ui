<template>
  <section :class="$cx(styles?.container({ prose: !notProse, bottomSpace }))">
    <div
      v-if="propControls?.length"
      :class="
        $cx(styles.controls({ background: controlsBg, prose: !notProse }))
      "
    >
      <div
      ...
      >
        <label
          :for="`prop-${prop.key}`"
          class="block text-xs font-medium text-gray-400 dark:text-gray-500 px-1"
        >{{ prop.label }}</label>
        <SenpCheckbox
        ...
        <SenpSelect
          ...
          :classes="{
            input: { extend: 'min-w-[6rem] !w-[calc(100%+1.5rem)]' },
          }"
        ...
        <SenpTextInput
          ...
          autocomplete="off"
          @update:model-value="
            (val) => (componentProps[prop.key] = tryParseOrDefault(val, val))
          "
        />
        <SenpTextInput
          v-else
          ...
          @update:model-value="
            (val) =>
              (componentProps[prop.key] =
                prop.type === 'number' ? Number(val) : val)
          "
        />
      </div>
    </div>
    <div
      :class="$cx(styles.canvas({ background: canvasBg, prose: !notProse }))"
    >
      <component
        :is="is"
        v-if="ignoreModel"
      ...
      >
        ...
        />
        <template
          v-for="slot in Array.isArray(slots)
            ? slots
            : Object.keys(slots || {})"
          :key="slot"
          #[slot]
    ...
    <div
      v-if="!noCode"
      :class="$cx(styles.code({ prose: !notProse, background: codeBg }))"
    >
      <ContentSlot
        v-if="$slots.code"
        :use="$slots.code"
...
import { tryParseOrDefault, reactive, computed, useAsyncData } from "#imports";
      ...
      .map((key) => {
        ...
        const type =
          typeof componentProps[key] === "number"
            ...
            ? "string"
            : typeof componentProps[key] === "object" &&
...
const styles = {
  ...
  controls: cva(
    "flex divide-x divide-gray-300 dark:divide-gray-700 max-w-full overflow-auto rounded-t-lg",
    {
      variants: {
        prose: { false: "not-prose", true: "prose" },
        background: {
