<template>
  ...
  >
    <div class="flex items-center gap-3 lg:max-w-7xl mx-auto w-full">
      ...
      >
        <p
          class="text-transparent bg-clip-text bg-gradient-to-br from-green-500 to-teal-700 text-4xl"
        >
          🎨
        </p>
        <p>SenpUI</p>
  ...
  <div
    ref="container"
    class="grid lg:grid-cols-[auto,minmax(0,1fr),auto] relative lg:max-w-7xl mx-auto w-full"
  >
    <nav
    ...
    >
      <ul
        v-for="dir in navigation"
        class="mb-4"
      >
        <p class="mb-2 capitalize text-sm font-medium">
          {{ dir.dir }}
        </p>
    ...
    <section class="w-full overflow-x-hidden relative">
      ...
      <div
        class="py-4 lg:py-8 mx-auto w-full"
        :class="{ 'mt-16 lg:mt-0': data?.body?.toc?.links?.length }"
      >
        <main
          id="prose-content"
        ...
        >
          <p class="capitalize text-sm font-medium text-blue-400">
            {{ activeDirectory }}
          </p>
          <ContentDoc />
          <hr>
          <div class="not-prose flex items-center gap-4">
            ...
            >
              ...
              <div class="grid gap-1">
                <p class="!m-0 font-medium text-base">
                  {{ pagination.prev.title }}
                </p>
                <p class="!m-0 text-sm text-gray-400">
                  {{ pagination.prev.description || "No Description" }}
                </p>
              </div>
            </NuxtLink>
            <NuxtLink
            ...
            >
              ...
              <div class="grid gap-1">
                ...
                <p class="!m-0 text-sm text-gray-400">
                  {{ pagination.next.description || "No Description" }}
                </p>
              </div>
            </NuxtLink>
          </div>
          <p class="py-12 text-sm text-gray-400">
            👩‍💻💚 nicole
          </p>
        </main>
      </div>
    ...
    <div
      class="hidden lg:block w-40 px-4 sticky top-16 py-8 max-h-[calc(100vh-4rem)] overflow-auto"
    >
      <DocsToc
        v-if="data?.body?.toc?.links?.length"
        :value="data?.body?.toc"
...
const navigation = computed(() => {
  ...
  navigationData.value.forEach((entry) => {
    ...
    if (groupedByDir.some((group) => group.dir === entry._dir)) {
      groupedByDir[
        groupedByDir.findIndex((group) => group.dir === entry._dir)
      ].children.push({
        path: entry._path,
        title: entry.title,
...
const pagination = computed(() => {
  ...
  const currentDirIndex = navigation.value.findIndex((val) =>
    val.children.some(
      (entry) => entry.path === (path.endsWith("/") ? path.slice(0, -1) : path)
    )
  );
