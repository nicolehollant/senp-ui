  ...
  <template v-else-if="typeOfValue === 'array'">
    ...
    >
      <div
        ...
        class="bg-slate-700/30 rounded-xl p-2 px-4"
      >
        <SenpAutoJson
...
type AutoJsonRenderPrimitiveOption =
  ...
  | { type: "code" }
  | { type: "link" };
type AutoJsonRenderOption =
  | AutoJsonRenderObjectOption
  | AutoJsonRenderArrayOption
  | AutoJsonRenderPrimitiveOption;
...
const renderStyleFromDotPath = computed(() => {
  ...
  try {
    ...
    if (
      renderAsStyle &&
      styles[typeOfValue.value].available.includes(renderAsStyle.type)
    ) {
    ...
    if (props.parentKey.split(".").some((part) => !isNaN(Number(part)))) {
      ...
      if (
        renderAsSlicedStyle &&
        styles[typeOfValue.value].available.includes(renderAsSlicedStyle.type)
      ) {
