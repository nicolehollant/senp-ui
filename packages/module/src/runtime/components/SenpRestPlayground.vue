  ...
  >
    <template #header>
      ...
      <div class="flex items-center gap-4">
        ...
        >
          <template #0>
            <Icon name="icon-park-outline:freeze-column" />
          </template>
          <template #1>
            <Icon
              name="icon-park-outline:freeze-column"
    ...
    <template #content>
      <div>
        <SenpDrawer
          ...
          :classes="{
            maxSize: {
              base: 'w-full max-h-full max-w-[min(95vw,448px)] md:max-w-4xl h-full',
            },
          }"
          side="left"
          :title="title"
          ...
          <aside class="sticky top-0">
            <nav>
              <button
                ...
                :class="{
                  'bg-gray-900':
                    route.method === state.activeMethod &&
                    baseurl + route.path === state.activeRoute,
                }"
                @click="
                  () => {
            ...
            >
              ...
              >
                <div
                  ...
                  :class="{
                    'flex flex-col w-1': state.view === 'horizontal',
                    'flex h-1': state.view === 'vertical',
                  }"
                >
                  <div
                    class="rounded bg-gray-600"
                    :class="{
                      'h-2': state.view === 'horizontal',
                      'w-2': state.view === 'vertical',
                    }"
                  />
                  <div
                    class="rounded bg-gray-600"
...
const setActiveRoute = (route: (typeof props.routes)[number]) => {
  ...
  state.value.activeRoute = props.baseurl
    ? props.baseurl + route.path
    : route.path;
...
const state = ref({
  ...
  activeRoute: "",
  activeMethod:
    props.routes?.[0]?.method ??
    ("GET" as (typeof props.routes)[number]["method"]),
...
if (props.queryKey) {
  useQueryParamSync(state, props.queryKey, {
    watchDeep: true,
    runOnMounted: true,
  });
}
