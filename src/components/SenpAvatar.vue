<template>
  <div :class="$cx(styles.wrapper(), classes?.wrapper)">
    ...
    <span
      ...
      :class="
        $cx(
          ...
          styles.avatar({ size, rounded, class: 'flex items-center justify-center' }),
          classes?.avatar
        )
      "
    >
    ...
    <div
      class="p-2"
      v-else
      :class="$cx(styles.avatar({ size, rounded, class: 'flex items-center justify-center' }), classes?.avatar)"
    >
      <Icon name="mdi:account" class="w-full h-full"></Icon>
    </div>
    <span
      v-if="chipColor"
      :class="$cx(styles.chip({ color: chipColor, position: chipPosition, size }), classes?.chip)"
    ></span>
  </div>
</template>
