...
export const useCsvFiltering = (data: any[], options: TOptions = {}) => {
  const state = ref<CsvState>(
    useQueryParamInitialValue(options.initialState, {
      ...
      tagPosition: 'top',
      settingsOpen: false,
      limit: options.limitTo != null ? Math.min(options.limitTo, data.length) + '' : '',
    })
  )
  ...
  const applySorterOp = (left: string, right: string, op: typeof sorterOperations[number]) =>
    ({
  ...
  const toSorter = ({ key, op }: CsvSorter) => {
    return (a: typeof data[0], b: typeof data[0]) => applySorterOp(a[key], b[key], op)
  }

  const sorters = ref<CsvSorter[]>(useQueryParamInitialValue(options.initialSorters, [], [Array.isArray]))

  const applyFilterOp = (left: string, op: typeof filterOperations[number], right: string) =>
    ({
      '=': (a: string, b: string) => {
        return a == b
      },
  ...
  const filters = ref<CsvFilter[]>(useQueryParamInitialValue(options.initialFilters, [], [Array.isArray]))
